...
class KEModel(object):
    ...
    def forward_test(self, pos_g, neg_g, logs, gpu_id=-1):
        ...
        ########### changes here ###########
        file_path = '/home/ubuntu/data/process/all-cvt-sub.txt'
        triples_set = set()

        with open(file_path, 'r') as file:
            for line in file:
                # Strip whitespace and split the line into components (subject, relation, object)
                parts = line.strip().split(',')
                # Ensure the line has exactly three components
                if len(parts) == 3:
                    subject, relation, object_ = parts
                    # Create a tuple for the triple and add it to the set
                    triples_set.add((subject.strip(), relation.strip(), object_.strip()))
        # Output the set of triples
        print("Extracted triplessss")
        ...
        ###########change 
        combined_mask = F.tensor([1.0] * neg_scores.numel(), dtype=F.float32).reshape(neg_scores.shape).to(F.context(neg_scores))
        ...
        if self.args.eval_filter:
            ...
            original_mask = filter_bias != -1
            combined_mask = combined_mask * original_mask
        ...
        # the positive edge.
        if triples_set is not None:
            #print("Node data keys:", neg_g.ndata.keys())
            #print("Edge data keys:", neg_g.edata.keys())

            neg_head_ids = neg_g.ndata['id'][neg_g.head_nid]
            neg_tail_ids = neg_g.ndata['id'][neg_g.tail_nid]
            rel_ids = neg_g.edata['_ID']
            neg_samples = list(zip(neg_head_ids.tolist(), rel_ids.tolist(), neg_tail_ids.tolist()))

            # Generate a filter mask: 1 for valid negatives, 0 for false negatives
            new_filter_mask = [(s, r, o) not in triples_set for s, r, o in neg_samples]
            new_filter_mask = F.tensor(new_filter_mask, dtype=F.float32).to(F.context(neg_scores))
            new_filter_mask = reshape(new_filter_mask, new_filter_mask.size(0), -1)

            if new_filter_mask.size(0) > combined_mask.size(0):
               new_filter_mask = new_filter_mask[:combined_mask.size(0)]  # Trim extra rows
            elif new_filter_mask.size(0) < combined_mask.size(0):
               padding = F.ones((combined_mask.size(0) - new_filter_mask.size(0),), dtype=new_filter_mask.dtype, ctx=F.context(new_filter_mask))
               padding = F.unsqueeze(padding, dim=1)
               new_filter_mask = F.cat([new_filter_mask, padding], dim=0)
            combined_mask = combined_mask * new_filter_mask  # Combine with the new mask

        # Apply the combined mask to the negative scores
        neg_scores = neg_scores * combined_mask
